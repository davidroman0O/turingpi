...
func (a *nodeAdapter) getSSHClient() (*ssh.Client, error) {
	...
	err := a.withRetry("establish SSH connection", func() error {
...
func (a *nodeAdapter) ExecuteCommand(command string) (stdout string, stderr string, err error) {
	...
	err = a.withRetry("execute command", func() error {
	...
	if err != nil {
		return stdout, stderr, fmt.Errorf("command '%s' failed: %w. Stderr: %s", command, err, stderr)
	}
...
func (a *nodeAdapter) ExpectAndSend(steps []InteractionStep, interactionTimeout time.Duration) (string, error) {
	...
	err = a.withRetry("expect and send interaction", func() error {
		session, err := client.NewSession()
		if err != nil {
			return fmt.Errorf("failed to create session: %w", err)
		}
